...
import { InputField, Variant } from "../../types";
...
const VoucherPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    voucher.input_fields.forEach((input: InputField) => {
    ...
    try {
      ...
      window.snap.pay(midtransToken, {
        ...
        onError: async () => {
          try {
            toast.error("Something Wrong PM Admin");
          } catch (error) {
    ...
    <div>
      ...
      {!voucher ? null : (
        <>
          ...
          <ContainerWrap className="mt-12 text-white">
            <Box className="relative pt-16 text-center">
              <div className="absolute top-[-40px] left-1/2 -translate-x-1/2">
                <img
                  ...
                  className="w-27 h-27 object-cover rounded-full border-4 border-white shadow-md"
                />
              </div>
            ...
            {voucher.input_fields.map((field: InputField) => (
            ...
            <Box className=" shadow rounded-xl p-4">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {voucher.variants.map((variant: Variant, index: number) => (
                  <button
                    key={index}
                    onClick={() => setSelectedVariant(index)}
                    className={`bg-button px-4 py-2 rounded-lg border cursor-pointer ${
